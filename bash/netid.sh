...
#####
while [ $# -gt 0 ]; do
	#check the first thing on the line and save it for use later
	case "$1" in 
		-v )
			verbose="yes"
			;;
		* )
			[ -v interfaceName ] || interfaceName="$1" 
			;; 
...
interface="ens33" #List of interface names 
intf=$(ls /sys/class/net) 

for face in $intf; do
[ -v interfaceName ] && interface=$interfaceName


[ "$verbose" = "yes" ] && echo "Reporting on interface(s): $face"

[ "$verbose" = "yes" ] && echo "Getting IPV4 address and name for interface $face"
# Find an address and hostname for the interface being summarized
# we are assuming there is only one IPV4 address assigned to this interface
ipv4_address=$(ip a s $face|awk -F '[/ ]+' '/inet /{print $3}')
ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')

[ "$verbose" = "yes" ] && echo "Getting IPV4 network block info and name for interface $face"
...
network_address=$(ip route list dev $face scope link|cut -d ' ' -f 1)
